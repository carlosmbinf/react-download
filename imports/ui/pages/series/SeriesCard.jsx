...
import { PelisCollection, SeriesCollection } from "../collections/collections";
...
import PeliCardOnly from "./SeriesCardOnly";
import SeriesCardOnly from "./SeriesCardOnly";
...
export default function SeriesCard(options) {
  ...
  const serie = useTracker(() => {
    ...
    if (options.clasificacion == "All") {
      return SeriesCollection.find({ mostrar: true }).fetch();
    } else {
      return SeriesCollection.find({ mostrar: true, clasificacion: options.clasificacion }, {
        fields: {
          _id: 1,
          clasificacion: 1,
          mostrar: 1,
  ...
  if (options.withCreate == "true") {
    ...
    <>
      <Fade top >
        ...
        >
          <Link to={"/create-series"} className={classes.link}>
  ...
  return serie.length > 0 ? <Fade left>
          <Grid
            container
            direction="row"
            justify="flex-start"
