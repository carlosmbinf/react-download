...
export default function SeriesCardOnly(options) {
  ...
  return <Link key={options.serieGeneral._id} to={"/series/" + options.serieGeneral._id} className={classes.link}>
          ...
          onMouseEnter={async () => {
            // console.log("Probando - " + mostrarTriler[options.serieGeneral._id]);
            try {

              Meteor.call("getUrlTrillerSeries",options.serieGeneral._id,(error,result)=>{
          ...
          onMouseLeave={() => {
            ...
            // mostrarTriler[options.serieGeneral._id] = false
            // console.log("Probando - " + mostrarTriler[options.serieGeneral._id]);
          ...
          >
            <Paper
              elevation={5}
              className={
                options.serieGeneral.mostrar !== "true"
                  ? classes.primary
                  : classes.secundary
              }
              style={{
                backgroundImage: "url('" + options.serieGeneral.urlBackgroundHTTPS + "')",
              }}
            >
              <Grid >
              ...
              <Grid className={classes.video} style={{ width: "100%", height:"100%", position: "absolute", bottom: 0, background: "black" }}>
                {/* INSERTAR VIDEO */}
                  <video autoPlay={true} width="100%" style={{ borderRadius: 20, width: "100%", height: "100%" }} poster={options.serieGeneral.urlBackgroundHTTPS} preload="metadata">
                    <source src={triller} type='video/mp4; codecs="avc1.42E01E, mp4a.40.2"' />
                    {/* <track default kind="subtitles" label="Español" src={`/getsubtitle?idPeli=${options.serieGeneral._id}`} srcLang="es" /> */}
