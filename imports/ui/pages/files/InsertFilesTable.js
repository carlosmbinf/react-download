...
export default function InsertFilesTable(option) {
  ...
  const precios = useTracker(() => {
    ...
    let a = [];
    Meteor.subscribe("files",option.selector?option.selector:{}).ready()&&   

    FilesCollection.find(option.selector?option.selector:{}, {
      sort: { createdAt: -1 }
    }).map(
      (data) =>
        data &&
        a.push({
          ...
          comentario: data.details && data.details
        })
    );
  ...
  const nombreBodyTemplate = (rowData) => {
    return (
      <React.Fragment>
        <span className="p-column-title">Nombre del File</span>
        {rowData.nombre}
      </React.Fragment>
    );
  };
  ...
  const eliminarBodyTemplate = (rowData) => {
    return (
      <React.Fragment>
        <span className="p-column-title"></span>
        <Tooltip
          title={`Eliminar ${rowData.nombre} CUP`}
        >
          <IconButton
            // disabled
            aria-label="delete"
