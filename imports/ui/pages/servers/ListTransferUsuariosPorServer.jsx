...
export default function ListTransferUsuariosPorServer() {
  ...
  const listaDeUsusarios = useTracker(() => {
    let ready = Meteor.user() && Meteor.subscribe("user",(Meteor.user().username == 'carlosmbinf') ? {} : {$or:[{bloqueadoDesbloqueadoPor:Meteor.userId()},{_id:Meteor.userId()}]},{fields:{_id:1,username:1}}).ready();
    let list = ready && Meteor.users.find((Meteor.user().username == 'carlosmbinf') ? {} : {$or:[{bloqueadoDesbloqueadoPor:Meteor.userId()},{_id:Meteor.userId()}]},{fields:{_id:1,username:1}}).fetch();
